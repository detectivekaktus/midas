...
class Event(Base):
    ...
    number of days from `last_run_on` until `next_run_on`.
    ...
    last_run_on = mapped_column(Date(), nullable=False)
    interval: Mapped[int] = mapped_column(Integer, nullable=False)
    next_run_on = mapped_column(Date(), nullable=False)

    user = relationship("User", back_populates="events")
    transaction_type = relationship("TransactionType", back_populates="events")

    def __repr__(self) -> str:
        return f"Event({self.id=!r}, {self.user_id=!r}, {self.transaction_type_id=!r}, {self.title=!r}, {self.description=!r}, {self.amount=!r}, {self.last_run_on=!r}, {self.interval=!r}, {self.next_run_on=!r})"
